...
public static class InstructorOnlyEndpointMatcher
{
    ...
    public static bool IsInstructorOnly(HttpRequest request)
    {
        ...
        if (HttpMethods.IsPost(method) && path.StartsWith("/api/sessions/", StringComparison.OrdinalIgnoreCase) && path.EndsWith("/questions/import", StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }
        if (HttpMethods.IsPost(method) && path.StartsWith("/api/sessions/", StringComparison.OrdinalIgnoreCase) && path.EndsWith("/questions", StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }
